...
**M14 是最新晋级的策略；M15 是最新完成的产品工作流。** M15 已经实现，不只是计划。
它新增 `arena-agent`：先运行配对种子联赛，再按照明确的 `balanced`（平衡）、
`yield`（产量）或 `safety`（安全）目标筛选策略；违反最低存活率或最大无效动作率
约束的策略会被明确拒绝；最终同时写出 JSON 和 Markdown 证据。M15 第一阶段已进入
`main`，项目目前暂停新特性开发，转入维护、文档和发布工作。
